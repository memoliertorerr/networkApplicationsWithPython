#Client Side HUI Chat Room
import tkinter, socket, threading
from tkinter import DISABLED, VERTICAL, END, NORMAL, Listbox
...
def verify_connection(name):
    ...
    if flag == "NAME":
        ...
        if message:
            ...
            #Create a thread to continuously reieve messages from the server
...
def recieve_message():
    ...
    while True:
        try:
            ...
            #An error occured, disconenct from the server
            my_listbox.insert(0, "Closing the connectiong.  Goodbye...")
            disconnect()
            break
...
port_entry = tkinter.Entry(info_frame, borderwidth=3, width=10, font=my_font)
connect_button = tkinter.Button(info_frame, text="Connect", width=8, font=my_font, bg=light_green, borderwidth=5, command=connect)
disconnect_button = tkinter.Button(info_frame, text="Disconnect", width=8, font=my_font, bg=light_green, borderwidth=5, state=DISABLED, command=disconnect)
...
send_button = tkinter.Button(input_frame, text="send", borderwidth=5, width=5, font=my_font, bg=light_green, state=DISABLED, command=send_message)
